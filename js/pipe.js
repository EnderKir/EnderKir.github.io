(function() {
  ...
  var Pipe = (Mario.Pipe = function(options) {
    this.pos = options.pos;

    // направление - это направление в котором мы двигаемся В трубу
  ...
  Pipe.prototype.isCollideWith = function(ent) {
    // Зачем вообще всё это? Потому что один 'разламывающийся предмет' -
    // мы рухнем как только уничтожим блок без этой линии.
    if (ent.pos === undefined) return; // не можем определить позицию - не проверяем))

    // Первые два элемента массива hitbox - это смещение, поэтому сразу найдём координаты
    var hpos1 = [
    ...
    // Если hitbox ещё и перекрываются
    if (
      !(
      ...
      if (
        ...
        //Если сущность находится над блоком, это в основном этаж
        ...
        if (Math.abs(hpos2[1] + ent.hitbox[3] - hpos1[1]) <= ent.vel[1]) {
          ...
          //Если сущность бьёт со стороны, то this - стена
